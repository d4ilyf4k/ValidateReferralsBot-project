from aiogram import Router, F, types
from database.db_manager import get_referral_link
from utils.keyboards import get_bank_kb, get_user_main_menu_kb

router = Router()

@router.message(F.text == "🏦 Выбрать банк")
async def choose_bank(message: types.Message):
    await message.answer(
        "Выберите банк, чтобы получить персональную ссылку на оформление карты:",
        reply_markup=get_bank_kb()
    )

@router.message(F.text.in_(["🏦Т-Банк", "🏦Альфа-Банк"]))
async def send_bank_info_and_link(message: types.Message):
    bank_key = "t-bank" if message.text == "🏦Т-Банк" else "alpha"

    if bank_key == "t-bank":
        desc = (
            "<b>🏦 Т-Банк (Тинькофф) | Дебетовая карта</b>\n\n"
            
            "<b>📋 Условия для получения бонуса:</b>\n\n"
            "•✅ Оформить <b>дебетовую карту Т-Банка</b>\n"
            "•✅ <b>Активировать карту</b> после получения — пополнить её на любую сумму\n"
                                    
            "<b>✨ Особые условия:</b>\n"
            "• <b>Совершить первую покупку</b> — онлайн или в магазине, на сумму от 500 рублей\n\n"
            
            "<b>💰 Что ты получаешь:</b>\n\n"
            "•✅ Все преимущества карты Т-Банка (проценты на остаток, кешбэк)\n"
            "•✅ Приятный дополнительный бонус в размере 500₽\n\n"
            
            "<b>⏱️ Сроки:</b>\n"
            "Бонус зачисляется автоматически в течение <b>5–10 дней</b> после выполнения условий.\n\n"
            "<i>📌 Бот будет отслеживать статус и напомнит другу о выплате твоего бонуса!</i>\n\n"
            "<b>Готов получить карту и бонус?</b>"
        )
    else:  # alpha
        desc = (
            "<b>🏦 Альфа-Банк | Дебетовая карта</b>\n\n"
                        
            "<b>📋 Условия для получения бонуса:</b>\n"
            "•✅ Оформить <b>дебетовую карту Альфа-Банка</b>\n"
            "•✅ <b>Получить и активировать карту</b> после оформления\n\n"
                        
            "<b>✨ Особые условия:</b>\n"
            "• <b>Совершить первую покупку</b> — онлайн или в магазине, на любую сумму\n\n"
            
            "<b>💰 Что ты получаешь:</b>\n"
            "•✅ Все преимущества карты Альфа-Банка (проценты, кешбэк, мили)\n"
            "•✅ <b>Дополнительный бонус в размере 500₽</b>\n\n"
            
            "<b>⏱️ Сроки:</b>\n"
            "Бонус зачисляется автоматически в течение <b>3–14 дней</b> после выполнения условий.\n\n"
            "<i>📌 Бот будет отслеживать статус и напомнит другу о выплате твоего бонуса!</i>\n\n"
            "<b>Готов получить карту и бонус?</b>"
        )
    await message.answer(desc, parse_mode="HTML")

    link = await get_referral_link(bank_key)
    if link:
        bank_name = "Т-Банка" if bank_key == "t-bank" else "Альфа-Банка"
        await message.answer(f"📎 Ваша ссылка для оформления карты {bank_name}:\n{link}")
    else:
        await message.answer("⚠️ Ссылка временно недоступна. Обратитесь к администратору.")

    await message.answer("Главное меню:", reply_markup=get_user_main_menu_kb())